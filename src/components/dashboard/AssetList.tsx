              ...
              {assets.map((asset, index) => (
                ...
                >
                  ...
                  <td className="text-right py-4 px-4 font-mono font-medium">
                    ${(asset.currentPrice ?? 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                  </td>
                  <td className="text-right py-4 px-4">
                    <div className={cn(
                      "flex items-center justify-end gap-1 font-medium",
                      (asset.changePercent24h ?? 0) >= 0 ? "price-positive" : "price-negative"
                    )}>
                      ...
                      <span>{(asset.changePercent24h ?? 0) >= 0 ? '+' : ''}{(asset.changePercent24h ?? 0).toFixed(2)}%</span>
                    </div>
                  </td>
                  <td className="text-right py-4 px-4">
                    <p className="font-medium">{asset.quantity}</p>
                    <p className="text-muted-foreground text-sm">@ ${(asset.avgPrice ?? 0).toFixed(2)}</p>
                  </td>
                  <td className="text-right py-4 px-4 font-mono font-medium">
                  ...
                  <td className="text-right py-4 px-4">
                    <p className={cn(
                      ...
                      (asset.profit ?? 0) >= 0 ? "price-positive" : "price-negative"
                    )}>
                      {(asset.profit ?? 0) >= 0 ? '+' : ''}${(asset.profit ?? 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </p>
                    <p className={cn(
                      "text-sm",
                      (asset.profitPercent ?? 0) >= 0 ? "price-positive" : "price-negative"
                    )}>
